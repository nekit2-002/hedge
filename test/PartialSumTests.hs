...
module PartialSumTests where

import CategAlgebra
import WriteReadSpec
import WriteReadTest (concatParams)
import PartialSumSpec
import Hedgehog.Internal.Runner (check)
import Hedgehog.Internal.Property
import System.TimeIt (timeIt)

class PartialSumSpec ioObj => PartialSumTester ioObj where
  partialSumTests :: IO [Bool]

instance PartialSumTester NamedIOSet where
  partialSumTests = (:) <$> (do
      putStrLn $ "\ESC[96m" ++ partialSum_n
      timeIt . check . withTests 1000 . property $ p) <*>
    mapM (\(pn, p) -> do
    putStrLn $ "\ESC[96m" ++ pn
    timeIt . check . withTests 1000 . property $ p) category_tests
    where
      (specs, sets) = partialSumSpec @NamedIOSet
      (partialSum_spec, partial_sump) = head specs
      cat_laws = tail specs
      ...
      (partialSum_n, p) = (partialSum_spec ++ "(IntArr)", partial_sump >>= assert)